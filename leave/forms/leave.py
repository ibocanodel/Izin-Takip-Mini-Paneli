...
class LeaveForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if start_date and end_date and start_date > end_date:
            self.add_error("start_date", "Bitiş tarihi, başlangıç tarihinden önce olamaz.")
        
        if (start_date and start_date < self.employee.work_start_date) or  (end_date and end_date < self.employee.work_start_date):
            self.add_error("start_date", "İzin tarihi işe başlangıç tarihinden önce olamaz")
        
        current_request_business_days = self.calculate_business_days(start_date ,end_date , working_days)
        if self.employee and start_date and end_date:
            ...
            if overlap_exists:
                self.add_error("start_date", "Bu tarihler arasında başka bir izin talebiniz bulunmaktadır.")
            start_period,period_start,period_end = self.get_leave_period_year(start_date)
            end_period, _ , _ = self.get_leave_period_year(end_date)
            if start_period != end_period:
                self.add_error("start_date", "Başlangıç ve bitiş tarihleri aynı izin döneminde olmalıdır.")
            ...
            for leave in leaves_in_current_leave_type:
                leave_type_count += self.calculate_business_days(leave.start_date, leave.end_date, working_days)
            if leave_type_count + current_request_business_days > leave_type.annual_leave_days:
                if settings.allow_exceed_total_leave_days and not self.force_submit:
                    self.add_error(None, "Bu izin türündeki toplam limiti aştınız.Devam etmek istiyor musunuz?")
                elif not settings.allow_exceed_total_leave_days:
                    self.add_error("start_date", "Bu izin türündeki toplam limiti aştınız")

            if leave_type.count_on_total_days:
                all_leaves = leaves.filter(leave_type__count_on_total_days=True)
                all_leave_count = 0
                for leave in all_leaves:
                    all_leave_count += self.calculate_business_days(leave.start_date, leave.end_date, working_days)
                if all_leave_count + current_request_business_days > settings.annual_leave_days:
                    if settings.allow_exceed_total_leave_days and not self.force_submit:
                        self.add_error(None, "Toplam izin limiti aştınız.Devam etmek istiyor musunuz?")
                    elif not settings.allow_exceed_total_leave_days:
