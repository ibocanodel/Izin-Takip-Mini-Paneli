...
def leave_request(request):
    ...
    if request.method == 'POST':
        form = LeaveForm(request.POST ,employee = current_employee , force_submit=request.POST.get("force_submit") == "true" )
    ...
    return render(request, 'leave_request.html', {'leave_types': leave_types, "work_days": working_days , "employee" : current_employee })
...
def all_leaves_view(request):
    ...
    leaves = Leave.objects.select_related("employee", "leave_type", "employee__department").filter(is_deleted=False)
...
def leave_list(request):
    ...
    years = Leave.objects.filter(employee_id=current_employee_id, is_deleted=False) \
                .annotate(year=ExtractYear("start_date")) \
                .values_list("year", flat=True).distinct().order_by("-year")

    return render(request, "employee_own_leave_list.html", {
        "leaves": leaves,
        "years": years,
